(async function() {
  ...
  function renderHorizontal(schedules) {
    ...
    schedules.forEach(s => {
      ...
      html += '<div class="tt-block-row"><div class="tt-block-h" style="left:' + left + '%;width:' + width + '%;background:' + bg + '" title="' + s.title + ' ' + f24(s.from) + '~' + f24(s.to) + '">' + s.title + '</div></div>';
    });
    ...
    schedules.forEach(s => {
      ...
      html += '<div class="tt-leg-item"><div class="tt-leg-dot" style="background:' + bg + '"></div><span>' + s.title + '</span><span class="tt-leg-time">' + s.from.substring(8,10) + ':' + s.from.substring(10,12) + '~' + s.to.substring(8,10) + ':' + s.to.substring(10,12) + '</span></div>';
    });
    html += '</div>';
  ...
  function renderVertical(schedules) {
    ...
    schedules.forEach(s => {
      ...
      html += '<div class="tt-v-item"><div class="tt-v-time">' + ft + '</div><div class="tt-v-info"><div class="tt-v-title">' + s.title + '</div><div class="tt-v-detail">' + ft + ' ~ ' + tt + ' | ' + (s.requester || '') + '</div><div class="tt-v-bar"><div class="tt-v-bar-fill" style="width:' + s.progress + '%;background:' + bg + '"></div></div></div></div>';
    });
    document.getElementById('ttContent').innerHTML = html;
  }
  ...
  function renderCard(schedules) {
    ...
    schedules.forEach(s => {
      ...
      html += '<div class="tt-card"><div class="tt-card-title">' + s.title + '</div><div class="tt-card-time">' + ft + ' ~ ' + tt + (s.requester ? ' | ' + s.requester : '') + '</div><div class="tt-card-bar"><div class="tt-card-bar-fill" style="width:' + s.progress + '%;background:' + bg + '"></div></div><div style="font-size:10px;color:var(--color-text-secondary);margin-top:2px">' + s.progress + '%</div></div>';
    });
    document.getElementById('ttContent').innerHTML = html;
  }
